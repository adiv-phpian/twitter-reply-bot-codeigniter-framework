		...
		$(document).ready(function() {
			...
			$('#example').DataTable( {
		        dom: "B<clear>frtip",
						bSortClasses: false,
					  ajax: {
		            url: "<?=base_url()?>products/tweets_data/?type=<?=$type?>&id=<?=$id?>",
		            type: "POST"
		        },
		       ...
		       columns: [
				      ...
				      { data: "product_name", searchable: false, orderable: false, render: function ( pname, type, row ) {
								 //var obj = JSON.stringify(tweet);
				         return '<a target="__blank" href="'+base_url+'products/product/'+row.product_id+'">'+pname+'</a>';
							}},
				         ...
				         return '<a id="tweet_detail_'+row.id+'">'+tweet+'</a>';
							...
							{ data: "datetime", "defaultOrder": true, searchable: false},
							<?php } ?>
							<?php if(in_array("source", $table_options)){ ?>

							{ data : "source", orderable: false, searchable: true, render: function ( source, type, row ) {
    ...
    <div style="">
			<h4 style="text-align: center;">
				<?php
					     ...
					     echo "All tweets for keyword ".$product->product_name;
		...
		<!-- Modal -->
			 ...
			 <div class="modal-dialog">
				 ...
				 <div class="modal-content">
					 ...
					 <div class="modal-body">
						 ...
						 <form id="export_form" method="post" action="<?=base_url()?>export_data/">
                   ...
                   <input name="keywords[]" id="text<?=$product->id?>" type="hidden" value="<?=$product->id?>" />
								 ...
								 <ul class="checkbox-grid">
									...
									<h4> Do you want to delete after export? </h4>
		 ...
		 $(function(){
			 ...
			 $("#delete_all_tweets").click(function(){
				 ...
				 var get = $.get("<?=base_url()?>dashboard/delete_all_tweets/<?=$id?>");
